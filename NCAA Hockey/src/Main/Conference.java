...
public class Conference implements Serializable {
    ...
    public ArrayList<PlayerHockey> allConferencePlayers;
        ...
        if (confNum != -1) {
            ...
            for (int offset = 3; offset < 6; ++offset) {
                ...
                for (int i = 0; i < 10; ++i) {
                    ...
                    if (offset == 3) {
                        teamA.nonConfGameSlot1 = gm;
                        teamB.nonConfGameSlot1 = gm;
                    } else if (offset == 4) {
                        teamA.nonConfGameSlot2 = gm;
                        teamB.nonConfGameSlot2 = gm;
                    } else if (offset == 5) {
                        teamA.nonConfGameSlot3 = gm;
                        teamB.nonConfGameSlot3 = gm;
                    }
    ...
    public void insertNonConferenceSchedule() {
        for (TeamHockey team : confTeams) {
            ...
            team.gameSchedule.add(0, team.nonConfGameSlot1);
            team.gameSchedule.add(4, team.nonConfGameSlot2);
            team.gameSchedule.add(9, team.nonConfGameSlot3);
        }
    }
    ...
    public void scheduleConferenceChampionship() {
        ...
        // Break possible ties. This code is analogous to the football approach, but you
        // might adjust tie-breakers for hockey (head-to-head, total points, etc.).
        breakThreeWayTie(0);
        breakThreeWayTie(1);
    ...
    /**
     * Utility function for tie-breaking. Adapt tie-break logic for hockey if needed.
     */
    private void breakThreeWayTie(int startIndex) {
        int topWins = confTeams.get(startIndex).getConfWins();
        ArrayList<TeamHockey> ties = new ArrayList<>();
        ties.add(confTeams.get(startIndex));
        int i = startIndex + 1;
        while (i < confTeams.size() && confTeams.get(i).getConfWins() == topWins) {
            ties.add(confTeams.get(i));
            i++;
        }
        if (ties.size() > 2) {
            // Sort the tie by poll, rating, or head-to-head cluster
            Collections.sort(ties, new TeamHockeyCompPoll());
            for (int j = 0; j < ties.size(); j++) {
                confTeams.set(startIndex + j, ties.get(j));
            }
        }
    }
    ...
    public void playConferenceChampionship() {
        ...
        if (confChampGame.homeScore > confChampGame.awayScore) {
            home.conferenceChampion = "CONF-CHAMP";
            home.totalConferenceTitles++;
            away.totalConfTitleLosses++;
            ...
            away.conferenceChampion = "CONF-CHAMP";
            away.totalConferenceTitles++;
            home.totalConfTitleLosses++;
            league.newsStories.get(13).add(
     ...
     */
    public ArrayList<PlayerHockey> getAllConferencePlayers() {
        if (allConferencePlayers.isEmpty()) {
            // Separate out players by position
            ArrayList<PlayerForward> forwards = new ArrayList<>();
            ArrayList<PlayerDefense> defensemen = new ArrayList<>();
            ArrayList<PlayerGoalie> goalies = new ArrayList<>();
...
class TeamHockeyCompConfWins implements Comparator<TeamHockey> {
    ...
    public int compare(TeamHockey a, TeamHockey b) {
        ...
        if ("CONF-CHAMP".equals(a.conferenceChampion)) return -1;
        if ("CONF-CHAMP".equals(b.conferenceChampion)) return 1;
